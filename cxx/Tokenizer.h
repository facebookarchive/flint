...
#ifndef CPPLINT_TOKENIZER_H_
#define CPPLINT_TOKENIZER_H_
...
struct Token {
  ...
  folly::StringPiece precedingWhitespace_;
  folly::StringPiece file_;
  size_t line_;

  Token() : line_(0) {}
  Token(TokenType type, folly::StringPiece value,
        folly::StringPiece file, size_t line,
        const char *& preTokenPtr, size_t & preTokenLen)
      : type_(type), value_(value)
      , precedingWhitespace_(folly::StringPiece(preTokenPtr, preTokenLen))
      , file_(file), line_(line)
    {
      // The comment is read destructively
      preTokenPtr = value.data() + value.size();
      preTokenLen = 0;
    }

  std::string toString() const {
    return folly::to<std::string>(facebook::flint::toString(type_), '\t',
                    value_, '\t', file_, ':', line_);
  }
};
 ...
 * into them.
 */
void tokenize(const std::string & input, const  std::string & filename,
              std::vector<Token> & output);

} /*namespace flint*/ } /*namespace facebook*/

#endif // CPPLINT_TOKENIZER_H_
